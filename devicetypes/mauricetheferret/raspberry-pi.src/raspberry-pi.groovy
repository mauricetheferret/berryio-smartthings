...
preferences {
        ...
        input("pin", "string", title:"Pin", description: "pin", defaultValue: "2", required: true, displayDuringSetup: true)
}

metadata {
	definition (name: "Raspberry Pi", namespace: "mauricetheferret", author: "James Long") {
		capability "Polling"
		capability "Refresh"
	...
	tiles {
        ...
        standardTile("door", "device.door", width: 1, height: 1) {
			state("closed", label:'Door 1', icon:"st.doors.garage.garage-closed", backgroundColor:"#79b821", action: "open")
			state("open", label:'Door 1', icon:"st.doors.garage.garage-open", backgroundColor:"#ffa81e", action: "close")
		}
        ...
        main "door"
        details(["temperature", "cpuPercentage", "memory" , "diskUsage", "door", "restart", "refresh"])
    }
}
...
def parse(String description) {
    ...
    if (result.containsKey("gpio_value_${pin}")) {
    	log.debug "gpio_value_${pin}: ${result.gpio_value_${pin}.toDouble().round()}"
        if (result.gpio_value_${pin}.contains("1")){
        	log.debug "gpio_value_${pin}: closed"
            sendEvent(name: "door", value: "closed")
            sendEvent(name: "switch", value: "off")
        } else {
        	log.debug "gpio_value_${pin}: open"
...
def open() {
	log.debug "Door 1 was opened"
    sendEvent(name: "door", value: "open")
    def uri = "/api_command/gpio_set_value/${pin}/0"
    postAction(uri)
}
...
def close() {
	log.debug "Door 1 was closed"
    sendEvent(name: "door", value: "closed")
    def uri = "/api_command/gpio_set_value/${pin}/1"
    postAction(uri)
}
